...
func drawColorMenu(screen [][]string) [][]string {
	title := "Color"
	str := strings.Repeat("─", SymbolColorWidth-len(title)-2) + "┤"
	utils.DrawString(1, 15, title, pixel.Yellow, screen)
	utils.DrawString(len(title)+2, 15, str, pixel.Gray, screen)
	for y, line := range Colors {
		switch line {
		case "r":
			utils.DrawString(colorX+2, y, strconv.Itoa(cursor.CC.Color.R), pixel.White, screen)
			utils.SetByKeys(colorX, y, utils.FgRgb(pixel.Color{R: cursor.CC.Color.R}, "█"), pixel.White, screen)
		case "g":
			utils.DrawString(colorX+2, y, strconv.Itoa(cursor.CC.Color.G), pixel.White, screen)
			utils.SetByKeys(colorX, y, utils.FgRgb(pixel.Color{G: cursor.CC.Color.G}, "█"), pixel.White, screen)
		case "b":
			utils.DrawString(colorX+2, y, strconv.Itoa(cursor.CC.Color.B), pixel.White, screen)
			utils.SetByKeys(colorX, y, utils.FgRgb(pixel.Color{B: cursor.CC.Color.B}, "█"), pixel.White, screen)
		}
	}
